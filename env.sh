...
if [[ "$SCRAM_ARCH" == "slc6"* ]]; then
    ...
    rootsetup=/cvmfs/sft.cern.ch/lcg/releases/LCG_88/ROOT/6.08.06/x86_64-slc6-gcc62-opt/ROOT-env.sh
    setup
# Old setup:
...
# New setup using ROOT-env.sh (which also sets python) and newer pip
elif [[ "$SCRAM_ARCH" == "slc7"* ]]; then
    gccsetup=/cvmfs/sft.cern.ch/lcg/contrib/gcc/7/x86_64-centos7/setup.sh
